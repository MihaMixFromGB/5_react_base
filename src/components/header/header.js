...
const settings = [
  { title: "Profile", to: "/profile", name: "profile" }, 
  { title: "Logout", to: "/", name: "logout" } 
];

export const Header = ({isAuth}) => {
  const pages = isAuth ? authorizedPages : notAuthorizedPages;
    ...
    <AppBar position="static">
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          >
            <Link className={styles.linkPage} to="/">{logo}</Link>
          </Typography>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages.map((page) => (
              ...
              >
                <Link className={styles.linkPage} to={page.to}>{page.title}</Link>
              </Button>
            ))}
          </Box>
